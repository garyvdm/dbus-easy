...
class ProxyInterface(BaseProxyInterface):
    ...
    another client for the GLib :class:`MessageBus <dbus_ezy.glib.MessageBus>`
    implementation.

    This class is not meant to be constructed directly by the user. Use
    :func:`ProxyObject.get_interface()
    <dbus_ezy.glib.ProxyObject.get_interface>` on a GLib proxy
    ...
    returns an error, it will be a :class:`DBusError <dbus_ezy.DBusError>`
    ...
    error, it will be a :class:`DBusError <dbus_ezy.DBusError>` with
...
class ProxyObject(BaseProxyObject):
    """The proxy object implementation for the asyncio :class:`MessageBus <dbus_ezy.aio.MessageBus>`.
